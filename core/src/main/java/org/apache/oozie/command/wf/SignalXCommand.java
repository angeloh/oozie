...
import org.apache.oozie.command.VerifyXCommandException;
...
public class SignalXCommand extends WorkflowXCommand<Void> {
    ...
    @Override
    protected void verifyPrecondition() throws CommandException, VerifyXCommandException {
        if ((wfAction == null) || (wfAction.isComplete() && wfAction.isPending())) {
            if (wfJob.getStatus() != WorkflowJob.Status.RUNNING
                ...
                throw new VerifyXCommandException(ErrorCode.E0812, wfJob.getStatusStr());
            }
        } else {
            throw new VerifyXCommandException(ErrorCode.E0813, actionId, wfAction.getStatusStr(), wfAction.isPending());
        }
    }
